...
#Import Information calibration file
filename = "videos/gopro/calibration_W.mp4" #"videos/gopro/calibration_W.mp4"
#Input the number of board images to use for calibration (recommended: ~20)
n_boards = 20
#Input the number of squares on the board (width and height), number of intersections
board_w = 10
board_h = 6
...
subdirectory = 'calibration_images_' + file
final_directory = os.path.join(current_directory, subdirectory )
if not os.path.exists(final_directory):
    os.makedirs(final_directory)

#The ImageCollect function requires two input parameters.  
#Filename is the name of the file in which checkerboard images will be collected from. 
#Press space during playback to save frames to use in calibration.
#n_boards is the number of images of the checkerboard which are needed.
...
def ImageCollect(filename, n_boards):
    ...
    if status == True:
        ...
        #FPS = video.get(cv2.CAP_PROP_FPS) #geeft 59.94...
        #print("FPS = ", FPS)
        FPS = 60
        ...
        #change current directory to the made subdirectory
...
#The ImageProcessing function performs the calibration of the camera based on the images collected during ImageCollect function.
#This function will look for the images in the folder which contains this file.
#The function inputs are the number of boards which will be used for calibration (n_boards), 
#the number of squares on the checkerboard (board_w, board_h) as determined by the inside points (i.e. where the black squares touch).  
...
def ImageProcessing(n_boards, board_w, board_h, board_dim):
    ...
    for i in range(1, n_boards + 1):
        ...
        if found == True:
            ...
            subdirectory = 'images_where_corners_are_found'
            new_directory = os.path.join(old_directory, subdirectory )
            if not os.path.exists(new_directory):
                os.makedirs(new_directory)
                
            #change current directory to the new subdirectory
            os.chdir(new_directory)
            
            #save the image
            good_images += 1
            cv2.imwrite('Calibration_Image' + str(good_images) + '.png', image)
            
            #switch back to the directory with the calibration images
    ...
    name = 'calibration_data_' + file
    np.savez(name, distCoeff=distCoeff, intrinsic_matrix=intrinsic_matrix)
